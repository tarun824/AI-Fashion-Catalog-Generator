...
class AIStylingService {
  ...
  colorTheory = {
    fair: {
      best: ['Jewel Tones', 'Pastels', 'Royal Blue', 'Emerald Green', 'Wine', 'Coral'],
      avoid: ['Pale Yellow', 'Beige'],
      description: 'Rich jewel tones and soft pastels complement fair skin beautifully',
    },
    wheatish: {
      best: ['Earth Tones', 'Warm Colors', 'Mustard', 'Olive Green', 'Burnt Orange', 'Deep Red'],
      avoid: ['Neon Colors', 'Very Light Pastels'],
      description: 'Warm earthy tones and rich colors enhance wheatish complexions',
    },
    dusky: {
      best: ['Bright Colors', 'White', 'Gold', 'Hot Pink', 'Electric Blue', 'Orange'],
      avoid: ['Very Dark Colors', 'Brown'],
      description: 'Vibrant bright colors and metallics look stunning on dusky skin',
    },
  };
  ...
  fabricRecommendations = {
    petite: {
      ...
      description: 'Light, flowing fabrics create graceful drapes without overwhelming',
    },
    average: {
      best: ['All Fabrics', 'Silk', 'Cotton', 'Georgette'],
      avoid: [],
      description: 'Most fabrics work well, choose based on occasion and preference',
    },
    'plus-size': {
      best: ['Structured Silk', 'Cotton Silk', 'Soft Georgette'],
      avoid: ['Very Clingy Fabrics', 'Too Stiff Fabrics'],
      description: 'Structured fabrics with good drape create elegant silhouettes',
    },
  };
  ...
  async analyzeCustomerPhoto(imageBuffer) {
    try {
      const base64Image = await embeddingService.prepareImageForAnalysis(imageBuffer);
...
Be objective and professional. Focus on what will help them look their best.`,
  ...
  generateRecommendations(analysis) {
    ...
    const fabricRecs = this.fabricRecommendations[bodyType] || this.fabricRecommendations.average;
  ...
  async getPersonalizedProducts(analysis, allProducts, limit = 20) {
    ...
    const scoredProducts = allProducts.map((product) => {
      ...
      const recommendedColors = recommendations.colors.recommended.map(c => c.toLowerCase());
      
      const colorMatch = productColors.some(color => 
        recommendedColors.some(rec => rec.includes(color.toLowerCase()) || color.toLowerCase().includes(rec))
      );
      if (colorMatch) score += 3;

      // Check fabric match
      const productFabric = (product.fabric || '').toLowerCase();
      const recommendedFabrics = recommendations.fabrics.recommended.map(f => f.toLowerCase());
      ...
      const recommendedOccasions = recommendations.occasions.map(o => o.toLowerCase());
